...
import "fpsmeter"
import "./due_frame/packer.js"
import "./proto/pb_common/pb_common_pb.js"
import "./proto/pb_lobby/pb_lobby_pb.js"
import "./proto/pb_rpsgame/pb_rpsgame_pb.js"
// import { Client } from "./due_frame/client.js";
import { Client } from "./due_frame/client";
import { Packer } from "./due_frame/packer.js"
import { ProtoEncoder } from "./due_frame/encoding/encoding.js"
...
function drawMatrix(x = canvas.width / 2, y = canvas.height / 2) {
    ...
    if (bot_status.gender === proto.pb_rpsgame.BotStatusReq.gender_type.WOMAN) {
...
function drawMatrixGuest(x = canvas.width / 2, y = canvas.height / 2, e_x, e_y, name, gender, city) {
    ...
    if (gender === proto.pb_rpsgame.BotStatusReq.gender_type.WOMAN) {
        ctx.fillStyle = "rgb(255,20,147)";
    } else {
        ctx.fillStyle = "rgb(0,191,255)";
    }
    ...
    for (var i in guest_bots) {
        if (i === bot_status.bot_id) {
            ...
            drawMatrixGuest(guest_bots[i].r_x + guest_bots[i].x - real_top_left_poi.x, guest_bots[i].r_y + guest_bots[i].y - real_top_left_poi.y,
                ...
                guest_bots[i].name, guest_bots[i].gender, "")
...
function registMessageHandler() {
    messageHandlers[proto.pb_common.ROUTE.LOBBY_ECHO] = {
        msg: proto.pb_lobby.EchoRes,
        handler:lobby_echo_handler
    };
    messageHandlers[proto.pb_common.ROUTE.RPSGAME_START] = {
        msg: proto.pb_rpsgame.BotStatusReq,
        handler:rpsgame_start_handler
    };
    messageHandlers[proto.pb_common.ROUTE.RPSGAME_PLAY] = {
        msg: proto.pb_lobby.EchoReq,
        handler:rpsgame_play_handler
    };
    messageHandlers[proto.pb_common.ROUTE.RPSGAME_RESULT] = {
        msg: proto.pb_lobby.EchoReq,
        handler:rpsgame_result_handler
    };
}

function onMessage(data) {
    if (data == "undefined") {
        console.error("onMessage data is undefined")
        return
    }
    if (messageHandlers[data.route] == null) {
        console.error("onMessage route is not exist", data.route)
        return
    }
    let route = data.route
    let uint8Array = data.data.slice(data.data.offset, data.data.limit).toBuffer();
    // let msg = messageHandlers[route].msg.deserializeBinary(uint8Array)
    messageHandlers[route].handler(uint8Array)
}

function lobby_echo_handler(data) {
    console.info("lobby_echo_handler", data)
}

function lobby_login_handler(data) {
    console.info("lobby_login_handler", data)
}

function match_echo_handler(data) {
    console.info("match_echo_handler", data)
}

function match_join_handler(data) {
    console.info("match_join_handler", data)
}

function rpsgame_echo_handler(data) {
    console.info("rpsgame_echo_handler", data)
}

function rpsgame_start_handler(data) {
    console.info("rpsgame_start_handler", data)
    let r = proto.pb_rpsgame.BotStatusRes.deserializeBinary(data)
    var bot_list = r.getBotStatusList();

    for (var i in bot_list) {
        // 如果收到广播连接断开，那么删除元素
        if (bot_list[i].getStatus() === proto.pb_rpsgame.BotStatusReq.status_type.CLOSE) {
...
function rpsgame_play_handler(data) {
    console.info("rpsgame_play_handler", data)
}

function rpsgame_result_handler(data) {
    console.info("rpsgame_result_handler", data)
}


function createWebSocket() {
    console.info('createWebSocket')
    let packer = new Packer({
        encoding: new ProtoEncoder(),
    });
    ...
    console.info('createWebSocket', ws)

    ws.onConnect(() => {
        console.log('onConnect');
        let req = new proto.pb_lobby.EchoReq();
        req.setMsg('hello');
        ws.send({
            route:proto.pb_common.ROUTE.LOBBY_ECHO,
            data: req
        });
        console.log('send');
        is_ws_open = true;
...
function createWebSocket2() {
    ws = new WebSocket("ws://" + location.hostname + ":3553/ws")
    ...
    ws.onmessage = function (evt) {
        var r = proto.pb_rpsgame.BotStatusReq.deserializeBinary(evt.data)
        var bot_list = r.getBotStatusList();
...
function sendStatusByWs(msg = '') {
    ...
    if (is_open || msg) {
        ...
        let chat = new proto.pb_rpsgame.BotStatusReq();
        ...
        ws.send({
            route:proto.pb_common.ROUTE.RPSGAME_START,
            data: chat
        })
...
function initLocalStorage() {
    ...
    if (gender !== null) {
        ...
        bot_status.gender = proto.pb_rpsgame.BotStatusReq.gender_type.MAN
    }
}
...
function initTools() {
    ...
    genderWoman.addEventListener('click', (evt) => {
        bot_status.gender = proto.pb_rpsgame.BotStatusReq.gender_type.WOMAN
        localStorage.setItem('star_gender', bot_status.gender);
    });
...
function welcome() {
    var str = [
        "1欢迎来到这个秘密的地方，茫茫人海，如果能在这里相遇，说明是一种缘分~",
        "互动方式如下：",
        "1. W A S D进行上下左右",
        "2. 空格开启聊天框，回车发送消息",
